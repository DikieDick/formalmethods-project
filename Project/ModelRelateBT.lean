...
-- lemmas needed to prove our final theorem
lemma Interp_iff_context_solvable (A B : Term Var):
〚 A 〛_{ρ,σ} = 〚 B 〛_{ρ,σ} ↔ ∀ C : Term Var -> Term Var, (solvable (C A) ↔  solvable (C B))
  := by
  sorry

lemma solvable_iff_hasHnf (A: Term Var):
solvable A ↔ hasHnf A
  := by
  sorry

lemma context_solvable_iff_BT (A B : Term Var):
( ∀ C : Term Var -> Term Var, (solvable (C A) ↔  solvable (C B))) ↔ BT A = BT B
  := by
  sorry

-- Our final theorem we wanted to state
